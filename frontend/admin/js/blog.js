...
/* ── STATE ──────────────────────────────────────── */
const BLOG_API = 'http://localhost:5000/api/v1/blogs';
let blogAllPosts = [];
...
function blogBootUI() {
  ...
  el('blogSidebarCount') && (el('blogSidebarCount').textContent = blogAllPosts.length);
  el('blogTotalCount') && (el('blogTotalCount').textContent = blogAllPosts.length);
  const now = new Date();
  el('blogFutureCount') && (el('blogFutureCount').textContent =
    blogAllPosts.filter(p => p.publishDate && new Date(p.publishDate) > now).length);
}
...
function blogDoSearch(q) {
  if (!q) { blogFilteredPosts = blogAllPosts; blogRenderCards(blogFilteredPosts); return; }
  const ql = q.toLowerCase();
  blogFilteredPosts = blogAllPosts.filter(p =>
...
function blogSortCards(val) {
  ...
  if (val === 'newest') arr.sort((a, b) => new Date(b.publishDate) - new Date(a.publishDate));
  if (val === 'oldest') arr.sort((a, b) => new Date(a.publishDate) - new Date(b.publishDate));
  if (val === 'title') arr.sort((a, b) => (a.title || '').localeCompare(b.title || ''));
  if (val === 'words') arr.sort((a, b) => (b.stats?.wordCount || 0) - (a.stats?.wordCount || 0));
...
function blogBuildCard(post, i) {
  ...
  const catClass = { festivals: 'b-tag-festivals', wellness: 'b-tag-wellness', traditions: 'b-tag-traditions' }[post.category] || 'b-tag-cat';
  const cmtCount = post.commentCount || (blogCommentCache[post._id] || []).length;
...
async function blogOpenComments(post) {
  ...
  el('cmLikes') && (el('cmLikes').textContent = (post.reactionCounts?.love || 0).toLocaleString());
  el('cmViews') && (el('cmViews').textContent = (post.viewCount || 0).toLocaleString());

  // Reset UI
  document.querySelectorAll('.cm-tab').forEach((t, i) => t.classList.toggle('active', i === 0));
  ...
  if (list) list.innerHTML = `<div class="cm-empty">
  ...
  el('cmCmtCount') && (el('cmCmtCount').textContent = (blogCommentCache[post._id] || []).length);
...
function blogRenderCommentList() {
  ...
  if (!list) return;
  if (!blogActivePost) { list.innerHTML = ''; return; }

  let comments = blogCommentCache[blogActivePost._id] || [];
  if (blogActiveFilter === 'visible') comments = comments.filter(c => !c.hidden);
  if (blogActiveFilter === 'hidden') comments = comments.filter(c => c.hidden);
  if (blogActiveFilter === 'replied') comments = comments.filter(c => c.adminReply);
...
function blogBuildCommentHTML(c) {
  const hiddenBadge = c.hidden ? `<span class="cm-badge cm-badge-hidden">Hidden</span>` : '';
  const repliedBadge = c.adminReply ? `<span class="cm-badge cm-badge-replied">Replied</span>` : '';
  const adminBadge = c.isAdmin ? `<span class="cm-badge cm-badge-admin">Admin</span>` : '';
  const heartedBadge = c.adminHearted ? `<span class="cm-badge cm-badge-admin" style="background:rgba(127,4,3,0.12)">♥ Hearted</span>` : '';
...
function blogToggleReplyBox(cid) {
  ...
  document.querySelectorAll('.cm-reply-box').forEach(b => b.classList.remove('open'));
  if (!isOpen) {
...
async function blogSendReply(cid) {
  ...
  if (!txt) { blogNotify('Please write a reply first.', 'error'); return; }
  if (!blogActivePost) return;

  try {
    await blogApiFetch(`${BLOG_API}/${blogActivePost._id}/comments/${cid}/reply`, {
      method: 'POST',
...
async function blogToggleHide(cid) {
  ...
  try {
    await blogApiFetch(`${BLOG_API}/${blogActivePost._id}/comments/${cid}/${action}`, {
      method: 'PATCH'
    });
    c.hidden = !c.hidden;
    blogRenderCommentList();
    blogNotify(c.hidden ? 'Comment hidden from public.' : 'Comment is now visible.', 'info');
  } catch (e) {
...
async function blogToggleHeart(cid) {
  if (!blogActivePost) return;
  try {
    const json = await blogApiFetch(`${BLOG_API}/${blogActivePost._id}/comments/${cid}/heart`, {
      method: 'PATCH'
    });
    const comments = blogCommentCache[blogActivePost._id] || [];
    const c = comments.find(x => x.id === cid);
    ...
    blogRenderCommentList();
    blogNotify(c && c.adminHearted ? 'Comment hearted ♥' : 'Heart removed.', 'success');
  } catch (e) {
...
function blogToggleDelConfirm(cid) {
  ...
  if (!box) return;
  const isOpen = box.classList.contains('open');
  document.querySelectorAll('.cm-del-confirm').forEach(b => b.classList.remove('open'));
...
async function blogDeleteComment(cid) {
  ...
  try {
    ...
    blogCommentCache[blogActivePost._id] = (blogCommentCache[blogActivePost._id] || []).filter(c => c.id !== cid);
...
async function blogPostAdminComment() {
  ...
  if (!txt) { blogNotify('Please write something first.', 'error'); return; }
  if (!blogActivePost) return;

  try {
    const json = await blogApiFetch(`${BLOG_API}/${blogActivePost._id}/comments`, {
      method: 'POST',
      body: JSON.stringify({ body: txt })
    });
    ...
    if (newComment) {
      ...
      if (!blogCommentCache[blogActivePost._id]) blogCommentCache[blogActivePost._id] = [];
      blogCommentCache[blogActivePost._id].unshift(mapped);
    }

    if (inp) inp.value = '';
    const cmtEl = document.getElementById('cmCmtCount');
    if (cmtEl) cmtEl.textContent = (blogCommentCache[blogActivePost._id] || []).length;
    blogRenderCommentList();
    blogRenderCards(blogFilteredPosts);
...
/* ── NOTIFICATION ────────────────────────────────── */
let blogNotifTimer;
const blogIconMap = { success: 'fa-circle-check', error: 'fa-circle-xmark', info: 'fa-circle-info' };
...
function blogOpenEditor(postId) {
  ...
  const targetUrl = postId ? `./blogpost.html?edit=${postId}` : './blogpost.html';
