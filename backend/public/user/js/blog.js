...
(function () {
  ...
  function blogGetExcerpt(post) {
    ...
    const txt = (post.search_text || post.rendered_html || "")
      .replace(/<[^>]+>/g, "")
      .trim();
  ...
  function blogGetCoverImg(post) {
    ...
    const imgBlock = (post.blocks || []).find(
  ...
  function blogShowGridError(msg) {
    ...
    el.style.cssText =
      "grid-column:1/-1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:48px 24px;text-align:center;gap:12px";
    el.innerHTML = `
  ...
  async function blogLoadFeaturedPosts() {
    ...
    try {
      const data = await blogApiFetch(
        `${BLOG_API_BASE}?featured=true&limit=6&sortBy=publishDate&order=desc`,
      );
  ...
  function blogBuildFeatSlides() {
    ...
    blogFeaturedPosts.forEach((f, i) => {
      ...
      const tags =
        f.tags && f.tags.length ? f.tags : [blogCatLabel(f.category)];
  ...
  function blogGoFeat(idx) {
    ...
    document
      .querySelectorAll(".blog-feat-dot")
      .forEach((d, i) => d.classList.toggle("active", i === blogFeatIdx));
    blogResetFeatAuto();
  }
  ...
  function blogRenderGrid(posts) {
    ...
    posts.forEach((p, i) => {
      ...
      const tags =
        p.tags && p.tags.length ? p.tags : [blogCatLabel(p.category)];
      const excerpt = blogGetExcerpt(p);
      const time =
        p.readTime ||
        `${Math.max(1, Math.ceil((p.stats?.wordCount || 0) / 220))} min`;
    ...
    if (total <= 5) {
      ...
      pages.forEach((p, idx) => {
        if (idx > 0 && p - pages[idx - 1] > 1)
          numsEl.appendChild(makeEllipsis());
  ...
  function blogInit() {
    ...
    if (featPrev)
      featPrev.addEventListener("click", () => blogGoFeat(blogFeatIdx - 1));
    if (featNext)
      featNext.addEventListener("click", () => blogGoFeat(blogFeatIdx + 1));
    ...
    if (viewport) {
      ...
      viewport.addEventListener("touchend", (e) => {
        ...
        if (Math.abs(dx) > 40)
          blogGoFeat(dx < 0 ? blogFeatIdx + 1 : blogFeatIdx - 1);
      });
    }
    ...
    const pgNext = document.getElementById("blogPgNext");
    if (pgPrev)
      pgPrev.addEventListener("click", () => blogGoToPage(blogCurrentPage - 1));
    if (pgNext)
      pgNext.addEventListener("click", () => blogGoToPage(blogCurrentPage + 1));
