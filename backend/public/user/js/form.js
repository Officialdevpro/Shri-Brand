...
document.addEventListener("DOMContentLoaded", () => {
  const BASE_URL = "https://shri-brand.onrender.com/api/v1";
  ...
  const getErrorEl = (field) =>
    field && document.getElementById(`${field.id}Error`);
  const show = (el) => {
  ...
  // ── Validators ────────────────────────────────────────
  const validateName = (f) =>
    f && f.value.trim().length >= 2 && f.value.trim().length <= 100;
  const validateMobile = (f) => f && /^\d{10}$/.test(f.value.trim());
  const validateEmail = (f) =>
    f && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(f.value.trim());
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showResponseMessage(
          data.message ||
            "Thank you for your inquiry! We will respond within 24–48 business hours.",
          "success",
        );
